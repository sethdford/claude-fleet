...
async function cmdEnableNative(_options: CliOptions): Promise<void> {
  console.log('Enabling native TeammateTool features...\n');

  // Check if sneakpeek is already installed
  try {
    execSync('which claudesp', { stdio: 'ignore' });
    console.log('  claudesp binary already available.\n');
  } catch {
    console.log('  Installing claude-sneakpeek...');
    try {
      execSync('npx @realmikekelly/claude-sneakpeek quick --name claudesp', {
        stdio: 'inherit',
      });
      console.log('\n  claude-sneakpeek installed.\n');
    } catch (error) {
      console.error('  Failed to install claude-sneakpeek:', (error as Error).message);
      console.error('\n  Manual installation:');
      console.error('    npx @realmikekelly/claude-sneakpeek quick --name claudesp');
      process.exit(1);
    }
  }

  // Verify native availability via the Fleet server
  try {
    const data = await fetch(`${_options.url}/coordination/status`);
    if (data.ok) {
      const status = await data.json() as { coordination: { native: { isAvailable: boolean } } };
      if (status.coordination.native.isAvailable) {
        console.log('  Native coordination: ACTIVE');
      } else {
        console.log('  Native coordination: Not detected yet (restart the server)');
      }
    }
  } catch {
    console.log('  Server not running. Start with: fleet start');
  }

  console.log('\n  Run flag monitor to check enabled features:');
  console.log('    ./scripts/flag-monitor.sh');
  console.log('\n  Spawn a native worker:');
  console.log('    fleet spawn scout-1 "List files" --spawn-mode native');
}

async function cmdCoordinationStatus(options: CliOptions): Promise<void> {
  const data = await request('GET', '/coordination/status', options);
  outputResult(data, options);
}

async function cmdMigrateCheck(options: CliOptions): Promise<void> {
  console.log('\n╔════════════════════════════════════════════════════╗');
  console.log('║       NATIVE COORDINATION HEALTH CHECK              ║');
  console.log('╚════════════════════════════════════════════════════╝\n');

  try {
    const data = await request('GET', '/coordination/health', options) as {
      healthy: boolean;
      adapter: string;
    ...
    for (const check of data.checks) {
      ...
      const suffix = check.passed ? '' : '  ← FIX THIS';
    ...
    if (data.healthy) {
      console.log('  ✓ All native coordination subsystems operational');
    } else {
      console.log('  ✗ Some subsystems need attention. Fix the failing checks above.');
    }

    console.log('');
  } catch (error) {
    console.error('Failed to check migration readiness:', (error as Error).message);
    console.error('Make sure the Fleet server is running and you have team-lead auth.');
    process.exit(1);
  }
}
...
function generateClaudeMd(projectDir: string, serverUrl: string): string {
  const projectName = projectDir.split('/').pop() ?? 'my-project';
  return `# Claude Fleet Agent Configuration

> Auto-generated by \`fleet init\`. This file configures Claude Code agents for fleet coordination.
...
You are a Claude Code agent operating as part of a coordinated fleet. Follow these rules:

### Coordination
1. **Always check for assigned tasks** before starting work: use \`team_tasks\` to see pending work
2. **Claim tasks** before working on them: use \`team_claim\` to prevent duplicate effort
3. **Report completion** when done: use \`team_complete\` with a summary of changes
4. **Check mail** for messages from other agents: use \`mail_read\` regularly

### Communication
- Use \`mail_send\` to communicate with other agents
- Use \`team_handoff\` when passing work to another agent
- Post status updates to the blackboard with \`blackboard_post\`
- Create checkpoints at meaningful milestones with \`checkpoint_create\`

### Safety
- Policy rules are enforced — operations matching block rules will be rejected
- Check \`policy_evaluate\` before running destructive commands
- Never force-push or delete protected branches
- Respect HITL (human-in-the-loop) requests — wait for approval when asked

### Work Items
- Track work with \`workitem_create\` and \`workitem_update\`
- Use batch operations for related items: \`batch_create\` + \`batch_dispatch\`

## Available MCP Tools (82)

### Team Management
\`team_status\` \`team_broadcast\` \`team_tasks\` \`team_assign\` \`team_complete\` \`team_claim\` \`team_spawn\` \`team_dismiss\` \`team_workers\` \`team_send\` \`team_handoff\`

### Work Items & Batches
\`workitem_create\` \`workitem_update\` \`workitem_list\` \`batch_create\` \`batch_dispatch\`

### Communication
\`mail_send\` \`mail_read\` \`blackboard_post\` \`blackboard_read\` \`blackboard_mark_read\` \`blackboard_archive\`
...
### Swarm Intelligence
\`swarm_create\` \`swarm_list\` \`swarm_kill\` \`swarm_broadcast\` \`pheromone_deposit\` \`pheromone_query\` \`pheromone_hot_resources\` \`belief_set\` \`belief_get\` \`belief_consensus\` \`credits_balance\` \`credits_leaderboard\` \`credits_transfer\` \`credits_history\`

### Governance
\`proposal_create\` \`proposal_list\` \`proposal_vote\` \`proposal_close\` \`bid_submit\` \`bid_list\` \`bid_accept\` \`bid_withdraw\` \`auction_run\` \`payoff_define\` \`payoff_calculate\`

### TLDR Summaries
\`tldr_get_summary\` \`tldr_store_summary\` \`tldr_get_codebase\` \`tldr_store_codebase\` \`tldr_dependency_graph\` \`tldr_stats\`
...
### Workflows
\`workflow_list\` \`workflow_get\` \`workflow_start\` \`execution_list\` \`execution_get\` \`execution_steps\` \`execution_pause\` \`execution_resume\` \`execution_cancel\` \`step_complete\` \`step_retry\`
`;
}
...
async function cmdInit(options: CliOptions): Promise<void> {
  ...
  console.log('Initializing Claude Fleet in this project...\n');

  // Check for existing file
  if (existsSync(claudeMdPath)) {
    const existing = readFileSync(claudeMdPath, 'utf-8');
    if (existing.includes('Claude Fleet Agent Configuration')) {
      console.log('  .claude/CLAUDE.md already exists (fleet-generated).');
      console.log('  Use --force to overwrite. Skipping.\n');
      // Still install MCP if needed
    } else {
      console.log('  .claude/CLAUDE.md already exists with custom content.');
      console.log('  Fleet config will be appended.\n');

      const fleetSection = generateClaudeMd(projectDir, options.url);
      const separator = '\n\n---\n\n';
      writeFileSync(claudeMdPath, existing + separator + fleetSection);
      console.log('  ✓ Fleet configuration appended to .claude/CLAUDE.md');
    }
  } else {
    ...
    writeFileSync(claudeMdPath, generateClaudeMd(projectDir, options.url));
    console.log('  ✓ Created .claude/CLAUDE.md with fleet agent configuration');
  }
  ...
  try {
    ...
    console.log('  ⚠ MCP auto-install skipped (Claude CLI not available)');
    console.log('    Run "fleet mcp-install" later to add MCP tools.');
  }

  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  Fleet initialization complete!');
  console.log('');
  console.log('  Next steps:');
  console.log('    1. Start the server:  fleet start');
  console.log('    2. Open Claude Code in this project');
  console.log('    3. Agents will auto-discover fleet tools');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
}
...
async function cmdSpawn(
  ...
  if (spawnMode) {
    const validModes = ['process', 'tmux', 'native'];
    if (!validModes.includes(spawnMode)) {
      console.error(`Invalid spawn mode: ${spawnMode}`);
      console.error('  Valid modes: process, tmux, native');
      process.exit(1);
    }
    body.spawnMode = spawnMode;
  }
  const data = await request('POST', '/orchestrate/spawn', options, body);
  outputResult(data, options);
}
...
WORKER COMMANDS
  ...
  spawn <handle> <prompt>             Spawn a new worker [team-lead]
    --spawn-mode <mode>                Mode: process (default), tmux, native
...
COMPOUND COMMANDS
  compound [options]                  Run autonomous compound iteration loop
    ...
    --objective <text>               Mission goal
    --port <port>                    Server port (default: 4800)
    --live                           Use real Claude Code workers

SETUP COMMANDS
  init                                Initialize fleet in current project
                                      Creates .claude/CLAUDE.md + installs MCP
...
NATIVE INTEGRATION COMMANDS
  enable-native                       Install claude-sneakpeek for native mode
  coordination-status                 Show coordination adapter status
  migrate-check                       Check native coordination subsystem health
...
EXAMPLES
  fleet init                          # Set up fleet in your project
...
COMPOUND EXAMPLES
  fleet compound                      # Run on cwd with defaults
  fleet compound --target-dir ~/proj  # Target a different project
  fleet compound --live --workers 2   # Real Claude Code workers
  fleet compound --max-iterations 5   # More fix cycles
...
async function main(): Promise<void> {
  ...
  try {
    switch (command) {
      ...
      case 'spawn':
        if (args.length < 2) {
          console.error('Usage: fleet spawn <handle> <prompt> [--spawn-mode process|tmux|native]');
          process.exit(1);
        }
        await cmdSpawn(options, args[0], args.slice(1).join(' '), values['spawn-mode'] as string | undefined);
        break;
      case 'dismiss':
        if (args.length < 1) {
      ...
      // Native Integration Commands
      case 'enable-native':
        await cmdEnableNative(options);
        break;
      case 'coordination-status':
        await cmdCoordinationStatus(options);
        break;
      case 'migrate-check':
      ...
      // Compound Runner
      case 'compound': {
        const { CompoundRunner } = await import('./compound/runner.js');

        const targetDir = resolve((values['target-dir'] as string) ?? process.cwd());
        const port = parseInt((values.port as string) ?? '4800', 10);

        const runner = new CompoundRunner({
          targetDir,
          maxIterations: parseInt((values['max-iterations'] as string) ?? '3', 10),
          numWorkers: Math.min(parseInt((values.workers as string) ?? '2', 10), 5),
          port,
          serverUrl: `http://localhost:${port}`,
          objective: (values.objective as string) ?? 'All quality gates pass with zero errors',
          isLive: values.live === true,
        });
